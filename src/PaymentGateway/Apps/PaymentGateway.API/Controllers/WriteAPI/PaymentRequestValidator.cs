...
namespace PaymentGateway.API.WriteAPI
{
    public class PaymentRequestValidator
    {
        ...
        {
            var reg = "^[0-9]{3}$";
            return !Regex.IsMatch(_paymentRequest.Card.Cvv, reg);
        }

        public  bool CardNumberInvalid()
        {
            var reg = "^[0-9]{4} [0-9]{4} [0-9]{4} [0-9]{4}$";
            return !Regex.IsMatch(_paymentRequest.Card.Number, reg);
        }

        public bool CardExpiryInvalid()
        {
            var reg = "^(0?[1-9]|1[012])/[0-9]{2}$";
            return !Regex.IsMatch(_paymentRequest.Card.Expiry, reg);
        }
    }
}